...
import { Button, Typography, Paper } from '@mui/material'; // Import Material-UI components
...
const { user } = useSelector((state) => state.auth);
  ...
  useEffect(() => {
    ...
    const fetchTransaction = async () => {
      try {
        const response = await fetch(`http://localhost:5555/api/transactions/user/${user?.uid}`); // Replace with your API endpoint
        if (!response.ok) {
  ...
  const generatePDF = () => {
    ...
    const lineWidth = 190; // Max width for the text
    ...
    y += 10;
    doc.text(`Amount: Rupees ${transaction.amount}`, (doc.internal.pageSize.getWidth() - doc.getTextWidth(`Amount: ₹${transaction.amount}`)) / 2, y , { maxWidth: lineWidth });
    y += 10;
    ...
    doc.text(thankYouText, (doc.internal.pageSize.getWidth() - doc.getTextWidth(thankYouText)) / 2, y, { maxWidth: lineWidth });
...
<br/>
          <Typography variant="h6">Thank for choosing Insurehub </Typography>
        </Paper>
      )}
    </div>
  );
